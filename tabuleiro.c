...
void exibirInfoTabuleiroMestre(void) {
  ...
  printf("Este programa simula o movimento de peças de xadrez usando técnicas avançadas:\n");
  printf("- Torre: %d casas para a direita (usando Recursividade)\n", TORRE_CASAS);
  printf("- Bispo: %d casas na diagonal para cima/direita (usando Recursividade e Loops Aninhados)\n",
         BISPO_CASAS);
  printf("- Rainha: %d casas para a esquerda (usando Recursividade)\n",
         RAINHA_CASAS);
  printf("- Cavalo: %d movimentos em \"L\" - 2 casas para cima e 1 para direita (usando Loops Complexos)\n",
         CAVALO_MOVIMENTOS);
  printf("\nCada movimento é representado por uma saída no console indicando a direção.\n");
  printf("Este desafio demonstra o uso de recursividade e loops complexos para simular movimentos.\n");
}